  ...
  table = $('.yajra-datatable').DataTable({        
      ...
      "columns": [
                    ...
                    { className: "col-lg-4 col-xs-4 col-sm-4 col-md-4 text-left" },
                    { className: "col-lg-4 col-xs-2 col-sm-2 col-md-2 text-left" },
      ...
      'language'    : {
        ...
        columns: [
            {data: 'nom_reg', className: 'col-lg-4 col-xs-4 col-sm-4 col-md-4 text-left', name: 'nom_reg'},
            {data: 'direccion', className: 'col-lg-4 col-xs-4 col-sm-4 col-md-4 text-left', name: 'direccion'},
            {data: 'telf_reg', className: 'col-lg-4 col-xs-2 col-sm-2 col-md-2 text-left', name: 'telf_reg'},
            {
              data: null,
              bSortable: false,
              mRender: function(data, type, value) {
                ...
                if (info.length!=-1) {
                  if (value["ver"]>0) {
                    return '<center><a class="btn btn-info btn-xs far fa-id-card" title="Mostrar registro" data-toggle="modal" data-target="#modalmostrar" onclick="buscarregion('+value["ver"]+');"></a></center>';  
                  } else {
            ...
            },            
            {
              ...
              mRender: function(data, type, value) {
                ...
                if (info.length!=-1) {                
                  if (value["editar"]>0) {
                    var xroute = "{{ route('region.edit',['region'=>'__idregion']) }}";
                    xroute = xroute.replace('__idregion', value["editar"]);
            ...
            },
            {
              ...
              mRender: function(data, type, value) {
                ...
                if (info.length!=-1) {                                
                  if (value["borrar"]>0) {
                    var xroute = "{{ route('region.destroy',['region'=>'__idregion']) }}";
                    xroute = xroute.replace('__idregion', value["borrar"]);
                    ...
                    doc['header'] = (function (page, pages) {
                        return {
                            table: {
                                ...
                                body: [
                                    [{ image: logo, width: 50 }],
                                    [{ text: 'Lista de regiones', alignment: 'center', fontSize: 14, bold: true, margin: [0, 10, 0, 0] }],
                                    [
                                        {
                                            text:
                                                [
                ...
                customize: function ( win ) {                  
                    ...
                    $(win.document.body)
                        ...
                        .prepend(                          
                            ...
                            '<br><br><div style="font-size:18px;">Dependencia: '+general_dep+'<br>Ubicación: '+general_ubi+'<br>Región: '+general_reg+'</div><div align="center" style="font-size:20px;">Lista de Regiones</div>'
                        );
    ...
    function buscarregion(id) {           
      ...
      $.ajax({
          ...
          success: function(response) {
              if (response!="Error") {
                  $('#datosregion').remove();            
                  $("#resultdatos").append(`<div id="datosregion" align="left">`+
                  ...
                  <h3 class="timeline-header"><a href="#">Teléfono</a>&nbsp;`+response.telf_reg+`</h3>
                  <div class="timeline-body">`+response.nom_reg+`</div>
                  ...
                  <div class="timeline-item">
                  <span class="time"><i class="fas fa-info"></i></span>
                  <h3 class="timeline-header no-border"><a href="#">Coordenadas</a>&nbsp;`+ response.coordenadas+`</h3>
                  </div>
                  </div>
                  <div>
                  ...
                  <div class="timeline-body">`+response.direccion+
