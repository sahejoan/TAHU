...
$(document).ready(function(){ 
  ...
  table = $('.yajra-datatable').DataTable({        
      ...
      "columns": [
                    { className: "col-xs-4 col-sm-4 col-md-4 text-left" },
                    { className: "col-xs-6 col-sm-6 col-md-6 text-left" },
                    { className: "col-xs-2 col-sm-2 col-md-2 text-left" },                                        
      ...
      'language'    : {
        ...
        deferRender: true,
        ajax:{
           url : '{{ route('firmaslist') }}',
           headers: {'X-CSRF-TOKEN': tokent},
           type: "POST",
           dataType: 'json'
         },
        columns: [
            {data: 'firma', className: 'col-xs-4 col-sm-4 col-md-4 text-left', name: 'firma'},

            {
              ...
              mRender: function(data, type, value) {
                ...
                if (info.length!=-1) {                
                  return value['nom_reg']+' - '+value['nom_dep']+' - '+value['ubicacion'];                  
                } else { return ''; }
              }
            },
            ...
            {
              data: null,
              className: 'col-xs-2 col-sm-2 col-md-2 text-center',
              bSortable: false,
              mRender: function(data, type, value) {
                var info = table.page.info();                 
                if (info.length!=-1) {                
                  if (value["actual"]==1) {
                    return '<input class="custom-control-input custom-control-input-danger custom-control-input-outline" type="checkbox" name="actual'+value['id']+'" id="actual'+value['id']+'" data-id="'+value['id']+'" value="'+value['id']+'" checked="" disabled>'+
                           '<label for="actual'+value['id']+'" class="custom-control-label">Si</label>';
                  } else {
                    return '<input class="custom-control-input custom-control-input-danger custom-control-input-outline" type="checkbox" name="actual'+value['id']+'" id="actual'+value['id']+'" data-id="'+value['id']+'" value="'+value['id']+'" disabled>'+
                           '<label for="actual'+value['id']+'" class="custom-control-label">No</label>';
                  }
                } else { return ''; }
              }
            },
            
            {
              ...
              mRender: function(data, type, value) {
                ...
                if (info.length!=-1) {                
                  if (value["editar"]>0) {
                    var xroute = "{{ route('firmas.edit',['firma'=>'__idfirma']) }}";
                    xroute = xroute.replace('__idfirma', value["editar"]);
            ...
            },
            {
              ...
              mRender: function(data, type, value) {
                ...
                if (info.length!=-1) {                                
                  if (value["borrar"]>0) {
                    var xroute = "{{ route('firmas.destroy',['firma'=>'__idfirma']) }}";
                    xroute = xroute.replace('__idfirma', value["borrar"]);
                    ...
                    doc['header'] = (function (page, pages) {
                        return {
                            table: {
                                ...
                                body: [
                                    [{ image: logo, width: 50 }],
                                    [{ text: 'Lista de firmas', alignment: 'center', fontSize: 14, bold: true, margin: [0, 10, 0, 0] }],
                                    [
                                        {
                                            text:
                                                [
                ...
                customize: function ( win ) {                  
                    ...
                    $(win.document.body)
                        ...
                        .prepend(                          
                            ...
                            '<br><br><div style="font-size:18px;">Dependencia: '+general_dep+'<br>Ubicación: '+general_ubi+'<br>Región: '+general_reg+'</div><div align="center" style="font-size:20px;">Lista de firmas</div>'
                        );
