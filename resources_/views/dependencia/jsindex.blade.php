  ...
  btnroute = "{{ route('dependencialist') }}";
  ...
  table = $('.yajra-datatable').DataTable({        
      ...
      "columns": [
                    ...
                    { className: "col-lg-3 col-xs-2 col-sm-2 col-md-2 text-left" },
                    { className: "col-lg-4 col-xs-4 col-sm-4 col-md-4 text-left" },                    
      ...
      'language'    : {
        ...
        deferRender: true,
        ajax:{
           url : '{{ route('dependencialist') }}',
           headers: {'X-CSRF-TOKEN': tokent},
           type: "POST",
           dataType: 'json'
         },
        columns: [
            {data: 'nom_dep', className: 'col-lg-4 col-xs-4 col-sm-4 col-md-4 text-left', name: 'nom_dep'},
            {data: 'nom_reg', className: 'col-lg-2 col-xs-2 col-sm-2 col-md-2 text-left', name: 'nom_reg'},
            {data: 'ubicacion', className: 'col-lg-2 col-xs-2 col-sm-2 col-md-2 text-left', name: 'ubicacion'},
            {data: 'nom_jefe', className: 'col-lg-4 col-xs-4 col-sm-4 col-md-4 text-left', name: 'nom_jefe'},            
            {
              data: null,
              bSortable: false,
              mRender: function(data, type, value) {
                ...
                if (info.length!=-1) {
                  if (value["ver"]>0) {
                    return '<center><a class="btn btn-info btn-xs far fa-id-card" title="Mostrar registro" data-toggle="modal" data-target="#modalmostrar" onclick="buscardependencia('+value["ver"]+');"></a></center>';  
                  } else {
            ...
            },            
            {
              ...
              mRender: function(data, type, value) {
                ...
                if (info.length!=-1) {                
                  if (value["editar"]>0) {
                    var xroute = "{{ route('dependencia.edit',['dependencium'=>'__iddependencium']) }}";
                    xroute = xroute.replace('__iddependencium', value["editar"]);
            ...
            },
            {
              ...
              mRender: function(data, type, value) {
                ...
                if (info.length!=-1) {                                
                  if (value["borrar"]>0) {
                    var xroute = "{{ route('dependencia.destroy',['dependencium'=>'__iddependencium']) }}";
                    xroute = xroute.replace('__iddependencium', value["borrar"]);
                    ...
                    doc['header'] = (function (page, pages) {
                        return {
                            table: {
                                ...
                                body: [
                                    [{ image: logo, width: 50 }],
                                    [{ text: 'Lista de dependencia', alignment: 'center', fontSize: 14, bold: true, margin: [0, 10, 0, 0] }],
                                    [
                                        {
                                            text:
                                                [
                ...
                customize: function ( win ) {                  
                    ...
                    $(win.document.body)
                        ...
                        .prepend(                          
                            ...
                            '<br><br><div style="font-size:18px;">Dependencia: '+general_dep+'<br>Ubicación: '+general_ubi+'<br>Región: '+general_reg+'</div><div align="center" style="font-size:20px;">Lista de dependencia</div>'                            
                        );
  ...
  function clickExportarExcel() {
    ...
    <script>  
    function buscardependencia(id) {           
      var route = "{{ route('verdependencia') }}";
      ...
      $.ajax({
          ...
          success: function(response) {            
              if (response!="Error") {
                  $('#datosdependencia').remove();            
                  $("#resultdatos").append(`<div id="datosdependencia" align="left">`+
                  ...
                  <span class="bg-green">Dependencia</span>
                  </div>              
                  <div>
                  <i class="fas fa-phone bg-blue"></i>
                  <div class="timeline-item">
                  <span class="time"><i class="fas fa-info"></i></span>
                  <h3 class="timeline-header"><a href="#">Teléfono</a>&nbsp;`+response.telf_dep+`</h3>
                  <div class="timeline-body">`+response.nom_dep+`</div>
                  ...
                  <div>
                  <i class="fas fa-user-tie bg-red"></i>
                  <div class="timeline-item">
                  <span class="time"><i class="fas fa-info"></i></span>
                  <h3 class="timeline-header no-border"><a href="#">Jéfe de la dependencia </a>&nbsp;</h3>
                  <div class="timeline-body">`+response.nom_jefedep+`</div>
                  </div>
                  </div>
                  <div>
                  ...
                  <div class="timeline-body">`+response.ubicacion+
