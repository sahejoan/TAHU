  ...
  btnroute = "{{ route('jefedependencialist') }}";
  ...
  table = $('.yajra-datatable').DataTable({        
      ...
      'language'    : {
        ...
        deferRender: true,
        ajax:{
           url : '{{ route('jefedependencialist') }}',
           headers: {'X-CSRF-TOKEN': tokent},
           type: "POST",
           dataType: 'json'
         },
        columns: [
            {data: 'nom_jefe', className: 'col-xs-12 col-sm-12 col-md-12 text-left', name: 'nom_jefe'},
            {
              data: null,
              bSortable: false,
              mRender: function(data, type, value) {
                ...
                if (info.length!=-1) {                
                  if (value["editar"]>0) {
                    var xroute = "{{ route('jefedependencia.edit',['jefedependencium'=>'__idjefedependencium']) }}";
                    xroute = xroute.replace('__idjefedependencium', value["editar"]);
            ...
            },
            {
              ...
              mRender: function(data, type, value) {
                ...
                if (info.length!=-1) {                                
                  if (value["borrar"]>0) {
                    var xroute = "{{ route('jefedependencia.destroy',['jefedependencium'=>'__idjefedependencium']) }}";
                    xroute = xroute.replace('__idjefedependencium', value["borrar"]);
                    ...
                    doc['header'] = (function (page, pages) {
                        return {
                            table: {
                                ...
                                body: [
                                    [{ image: logo, width: 50 }],
                                    [{ text: 'Lista de jefes', alignment: 'center', fontSize: 14, bold: true, margin: [0, 10, 0, 0] }],
                                    [
                                        {
                                            text:
                                                [
                ...
                customize: function ( win ) {                  
                    ...
                    $(win.document.body)
                        ...
                        .prepend(                          
                            ...
                            '<br><br><div style="font-size:18px;">Dependencia: '+general_dep+'<br>Ubicación: '+general_ubi+'<br>Región: '+general_reg+'</div><div align="center" style="font-size:20px;">Lista de jefes</div>'                            
                        );
