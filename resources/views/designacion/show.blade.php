...
{!!Form::close()!!}
  ...
  <section class="section">
    <div class="section-header">
      <h3 class="page__heading">Monitor de Actividades de Designaciones</h3>
    </div>        
    <br>
    ...
    <div class="row">
      <div class="col" align="left">
        <div class="pagination justify-content-start">
          <a class="btn btn-success" href="{{route('designacion.index')}}">Atras</a>
        </div>
      </div>      
    </div> 
  </section>                                      
...
@section("scripts")
  ...
  @include('designacion.jsshow')

  <script>
  var table;        
  $(document).ready(function(){ 
  btnroute = "{{ route('designacionlistmonitor') }}";
  ...
  table = $('.yajra-datatable').DataTable({        
      ...
      'language'    : {
        ...
        deferRender: true,
        ajax:{
           url : '{{ route('designacionlistmonitor') }}',
           headers: {'X-CSRF-TOKEN': tokent},
           type: "POST",
           dataType: 'json'
         },
        columns: [
            {data: 'numdesig', className: 'col-sm-2 text-left', name: 'numdesig'},            
            {
              data: null,
              bSortable: true,
              ...
              mRender: function(data, type, value) {
                if (value['fecha_desig'] != null) {
                  mf = moment(new Date(value['fecha_desig']));
                  fechadesig = mf.format('DD/MM/YYYY'); 
                  return fechadesig;  
                } else{
                  return '';
                }
            ...
            },            
            {
              ...
              className: 'col-sm-2 text-left',
              mRender: function(data, type, value) {
                if (value['fecha_cese'] != null) {
                  mf = moment(new Date(value['fecha_cese']));
                  fechacese= mf.format('DD/MM/YYYY'); 
                  return fechacese;  
                } else{
                  return '';
                }
            ...
            {data: 'funcionarios.apellido', className: 'col-sm-2 text-left', name: 'apellido'},            
            {data: 'funcionarios.nombre', className: 'col-sm-2 text-left', name: 'nombre'},            
            {
              data: null,
              bSortable: true,
              className: 'col-sm-2 text-left',
              mRender: function(data, type, value) {
                return value['dependencia']['nom_dep'];
              }
            },                                                              
            {
              data: null,
              ...
              className: 'col-sm-2 text-left',
              mRender: function(data, type, value) {
                return value['dependencia']['region']['nom_reg'];
              }
            },
            {
              data: null,
              ...
              className: 'col-sm-2 text-left',
              mRender: function(data, type, value) {
                return value['dependencia']['ubicacion'];
              }
            },
            {
              data: null,
