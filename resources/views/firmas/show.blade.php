...
{!!Form::close()!!}
  ...
  <section class="section">
    <div class="section-header">
      <h3 class="page__heading">Monitor de Actividades de Firmas</h3>
    </div>        
    <br>
    ...
    <div class="row">
      <div class="col" align="left">
        <div class="pagination justify-content-start">
          <a class="btn btn-success" href="{{route('firmas.index')}}">Atras</a>
        </div>
      </div>      
    </div> 
  </section>                                      
...
@section("scripts")
  ...
  @include('firmas.jsshow')

  <script>
  var table;        
  $(document).ready(function(){ 
  btnroute = "{{ route('firmaslistmonitor') }}";
  ...
  table = $('.yajra-datatable').DataTable({        
      ...
      'language'    : {
        ...
        deferRender: true,
        ajax:{
           url : '{{ route('firmaslistmonitor') }}',
           headers: {'X-CSRF-TOKEN': tokent},
           type: "POST",
           dataType: 'json'
         },
        columns: [
            {data: 'firma', className: 'col-sm-2 text-left', name: 'firma'},            
            {
              data: null,
              bSortable: true,
              className: 'col-sm-2 text-left',
              mRender: function(data, type, value) {
                return value['dependencia']['nom_dep']+" - "+value['dependencia']['region']['nom_reg']+" - "+value['dependencia']['ubicacion'];
              }
            },                   
            {
              data: null,
